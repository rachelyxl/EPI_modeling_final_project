...
# the reporduction number R0 is 1.31, 1.23, 1.08. R0 = beta/(mu + gamma). beta = R0 * (gamma + mu), 0.41
...
legend("topright", legend = c("With Covid-19 impact", "Without Covid-19 impact"),
       col = c("blue", "red"), lty = 1)

```


# use ggplot and convert X axis to dates with verticle lines of 3 stages.
```{r}
...
# Plot the infected people vs time with dates on the x-axis
plot(result_all_phases$date, result_all_phases$I,
     type = "l",
     xlab = "Date",
     ylab = "Number of Infected People",
     main = "Infected People vs the simulation with and without Covid-19 impact and real data",
...
# Plot the second line
lines(result_no$date, result_no$I,
      col = "red")
...
ggplot() +
  ...
  labs(x = "Date", y = "Number of Cases",
       title = "Infected People vs the simulation with and without Covid-19 impact and real data") +
...
plot(result_no[, "time"], result_no[, "I"] / N *100,
     ...
     main = "% Infected People vs Time in the simulation without Covid-19 impact",
     col = "blue",
     ylim = c(0, 5))
```
...
# the calculation to Rt
```{r}
# Calculate Rt for each time point
Rt_values <- 1.31 * params_phase1["Cj"] * (result_all_phases[, "S"] / params_phase1["N"])

# Update Rt values for Phase 2
Rt_values[time_phase2] <- 1.23 * params_phase2["Cj"] * (result_all_phases[time_phase2, "S"] / params_phase2["N"])

# Update Rt values for Phase 3
Rt_values[time_phase3] <- 0.75 * params_phase3["Cj"] * (result_all_phases[time_phase3, "S"] / params_phase3["N"])
